...
- LEMBRETE: O estado de morte do player, deve leva-lo para outra room, com uma animação especifica e uma tela especifica, semelhante ao blasphemous (feito) *falta só colocar um texto de morte ou coisa do tipo* (feito)

- FAZER AGORA ANIMAÇÕES PRÓPRIAS DO PLAYER para prosseguir com o processo de criação 
- IDLE (feito) - RUN (feito) - JUMP (Subindo, e caindo) - (feito) - ATTACK  1 (feito), 2 (feito), 3 (feito) - DAMAGE (feito) - PARRY (feito) - DEATH (feito) - WALL JUMP (feito) - DASH - (feito)
